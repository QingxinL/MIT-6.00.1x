...
def isValidWord(word, hand, wordList):
    ...
    for char in word:
        for element in hand_copy.keys():
            if char == element and hand_copy[element]!=0 :
                hand_copy[element]-=1
                charInHand=True
                break
            else:
    ...
    if charInHand == True and wordInList == True:
        return True
    return False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while (calculateHandlen(hand)!=0):
        print('Current Hand: ', getCurrentHand(hand))
        print()
        #displayHand(hand)
...
def getCurrentHand(hand): # Helper Method
    currentHand = ''
    for letter in hand.keys():
        for j in range(hand[letter]):
             #print(letter,end=" ")       # print all on the same line
